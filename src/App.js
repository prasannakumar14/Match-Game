...
const getImage = imagesList[0].imageUrl
...
class App extends Component {
  ...
  onClickImage = (url, refUrl) => {
    if (url === refUrl) {
      const shuffled = imagesList[Math.floor(Math.random() * imagesList.length)]
      const getNewImage = shuffled.imageUrl

      this.setState(prevState => ({
        score: prevState.score + 1,
        reference: getNewImage,
      }))
    }
    if (url !== refUrl) {
  ...
  renderImages = () => {
    const {ActiveTab, reference} = this.state
    const imageUrlList = imagesList.filter(
      everyimage => everyimage.category === ActiveTab,
    )
    return (
      <div className="image-container">
        <ul>
          <li>
            <img src={reference} alt="match" className="main-image" />
          </li>
        </ul>

        <ul className="ordered-list-buttons">
          {tabsList.map(every => (
            <li key={every.tabId} className="button-list">
              <button
                type="button"
                className={ActiveTab === every.tabId ? 'gold' : 'button'}
                onClick={() => this.onClickButton(every.tabId)}
              >
                {every.displayText}
              </button>
            </li>
        ...
        <ul className="ordered-list-images">
          {imageUrlList.map(everyImage => (
            <li key={everyImage.id} className="button-list">
              <button type="button" className="button-images">
                <img
                  src={everyImage.thumbnailUrl}
                  alt="thumbnail"
                  className="thumbnail-image"
                  onClick={() =>
                    this.onClickImage(everyImage.imageUrl, reference)
                  }
                />
              </button>
            </li>
      ...
      <div className="score-card-container">
        <div className="score-card">
          <img
            ...
            className="trophy"
          />
          <p className="card-score">YOUR SCORE</p>
  ...
  playAgain = () => {
    this.setState({score: 0, timer: 60, game: false})
  ...
  render() {
    const {score, timer, game} = this.state
    let countdown = ''
    if (game) {
      countdown = 0
    } else {
    ...
    return (
      <div className="match-container">
        ...
        {countdown !== 0 ? this.renderImages() : this.renderGameOver()}
      </div>
    )
  }
}
